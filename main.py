...
def binaryToDecimal(n):
  ...
  if baseCheck(n, 2) == False:
    return sys.exit(
      print(color.RED + "Input must be a binary number string [0-1]"))
...
def decimalToBinary(n):
  ...
  if baseCheck(n, 10) == False:
    return sys.exit(
      print(color.RED + "Input must be a decimal number string [0-9]"))
  if (int(n) <= 1):
    return n
...
prompt = input(color.LIGHTRED_EX +
               "\nType a number corrasponding to an option below [1-3]." +
               color.LIGHTCYAN_EX +
               "\n\n1. Binary to Decimal\n2. Decimal to Binary\n3. Close\n: " +
               color.RESET)
...
if int(prompt) == int(prompt) > 3:
  sys.exit(color.RED +
           "Please restart the program and choose a valid option [1-3]" +
           color.RESET)
